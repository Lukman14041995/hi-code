                    ...
                    <div class="box-header no-print">
                        <div class="box-tools">
                            ...
                            @if (auth()->user()->can('job_sheet.edit'))
                                <a href="{{ action([\Modules\Repair\Http\Controllers\JobSheetController::class, 'edit'], [$job_sheet->id]) }}"
                            ...
                            <a class="btn btn-success"
                                href="{{ action([\Modules\Repair\Http\Controllers\JobSheetController::class, 'print'], [$job_sheet->id]) }}"
                                target="_blank">
                            ...
                            <a class="btn btn-warning"
                                href="{{ action([\Modules\Repair\Http\Controllers\JobSheetController::class, 'printLabel'], [$job_sheet->id]) }}"
                                target="_blank">
                                <i class="fas fa-barcode"></i>
                                @lang('repair::lang.print_label')
                            </a>
                            <button type="button" class="btn btn-success submit_button" onclick="sendWhatsapp();">
                        ...
                        <table class="table table-bordered" style="margin-top: 15px;">
                            <tr>
                                <th rowspan="3">
                                    ...
                                    <span style="font-weight: 100">
                                        ...
                                        <input type="hidden" id="tanggal" value="{{ $job_sheet->created_at ?? '' }}">
                                    </span>
                                </th>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <input type="hidden" id="no_jobsheet" value="{{ $job_sheet->job_sheet_no ?? '' }}">
                                    <input type="hidden" id="mobile" value="{{ $job_sheet->customer->mobile ?? '' }}">
                                    <input type="hidden" id="nama_kustomer"
                                        value="{{ $job_sheet->customer->name ?? '' }}">
                                    <input type="hidden" id="estimasi"
                                        value="{{ number_format($job_sheet->estimated_cost ?? 0, 0, ',', '.') }}">
                                    <input type="hidden" id="status" value=" {{ $job_sheet->status?->name }}">
                                    <input type="hidden" id="catatan" value=" {{ $job_sheet->comment_by_ss }}">
                                    @php
                                        $product_condition = json_decode($job_sheet->product_condition, true);
                                      // Pastikan $defects didefinisikan dengan benar
                                    @endphp
                                    @if (!empty($product_condition))
                                        @foreach ($product_condition as $index => $product_cond)
                                            <input type="hidden" id="kelengkapan{{ $index }}"
                                                value="{{ $product_cond['value'] }}">
                                        @endforeach
                                    @endif
                                    <input type="hidden" id="total_kelengkapan" value="{{ count($product_condition) }}">
                                    @php
                                        $keluhans = json_decode($job_sheet->defects, true);
                                    @endphp
                                    @if (!empty($keluhans))
                                        @foreach ($keluhans as $index => $product_defect)
                                            <input type="hidden" id="keluhan{{ $index }}"
                                                value="{{ $product_defect['value'] }}">
                                        @endforeach
                                    @endif
                                    <input type="hidden" id="total_keluhan" value="{{ count($keluhans) }}">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <input type="hidden" id="merk" value="{{ $job_sheet->brand?->name ?? '' }}">
                                    ...
                                    <input type="hidden" id="model"
                                        value="{{ $job_sheet->deviceModel?->name ?? '' }}">

                                    <br>
                                    <b>@lang('repair::lang.serial_no'):</b>
                                    {{ $job_sheet->serial_no }}
                                    <input type="hidden" id="seri" value="{{ $job_sheet->serial_no ?? '' }}">
                            ...
                            <tr>
                                ...
                                <td>
                                    @php
                                        $checklists = [];
                                        if (
                                            !empty($job_sheet->deviceModel) &&
                                            !empty($job_sheet->deviceModel->repair_checklist)
                                        ) {
                                            $checklists = explode('|', $job_sheet->deviceModel->repair_checklist);
                                        }
                                        if (!empty($repair_settings['default_repair_checklist'])) {
                                            $checklists = array_merge(
                                                explode('|', $repair_settings['default_repair_checklist']),
                                                $checklists,
                                            );
                                        }
                                    @endphp
...
@section('javascript')
    <script>
        function getAllKelengkapan() {
            var totalKelengkapan = document.getElementById('total_kelengkapan').value; // Mengambil jumlah total kelengkapan
            var KelengkapanArray = [];

            for (var i = 0; i < totalKelengkapan; i++) {
                var kelengkapanId = 'kelengkapan' + i; // Membuat ID dinamis berdasarkan indeks
                var kelengkapanValue = document.getElementById(kelengkapanId).value; // Mengambil nilai kelengkapan
                KelengkapanArray.push(kelengkapanValue); // Menambahkan ke array
            }

            return KelengkapanArray;
        }

        function getAllKeluhan() {
            var totalKeluhan = document.getElementById('total_keluhan').value; // Mengambil jumlah total keluhan
            var keluhanArray = [];

            for (var i = 0; i < totalKeluhan; i++) {
                var keluhanId = 'keluhan' + i; // Membuat ID dinamis berdasarkan indeks
                var keluhanValue = document.getElementById(keluhanId).value; // Mengambil nilai keluhan
                keluhanArray.push(keluhanValue); // Menambahkan ke array
            }
        ...
        function sendWhatsapp() {
            var nama_kustomer = document.getElementById('nama_kustomer').value;
            var tanggal = document.getElementById('tanggal').value;
            var no_jobsheet = document.getElementById('no_jobsheet').value;
            var model = document.getElementById('model').value;
            var status = document.getElementById('status').value;
            var merk = document.getElementById('merk').value;
            var seri = document.getElementById('seri').value;
            var keluhan = getAllKeluhan();
            var kelengkapan = getAllKelengkapan();
            var catatan = document.getElementById('catatan').value;
            var estimasi = document.getElementById('estimasi').value;
            var mobile = document.getElementById('mobile').value;
            var message =
                `Hello, *${nama_kustomer}*\n\n` +
                `Berikut ini detail tanda terima transaksi di TOLERANT SMARTPHONE:\n\n` +
                `*Tanggal Order:* ${tanggal}\n\n` +
                `*No Order:* ${no_jobsheet}\n\n` +
                ...
                `*Down Payment: 0* \n\n` +
                `*Note:* Tanda terima ini dapat digunakan untuk pengambilan service di TOLERANT SMARTPHONE.`;

            var encodedMessage = encodeURIComponent(message);
            var whatsappUrl = `https://api.whatsapp.com/send?phone=${mobile}&text=${encodedMessage}`;
